...
%     base link (the fixed vertical link)
    baseX = [basePointX,baseLength*cosd(baseAngle)];% x coordinates of base link
    ...
    distX = [proxX(2),distLength*cosd(distAngle+proxAngle+baseAngle) + proxLength*cosd(proxAngle+baseAngle) + baseLength*cosd(baseAngle)];% x coordinates of distal link
